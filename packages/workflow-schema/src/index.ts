...
export const concurrencySettingsSchema = z
  .object({
    ...
    overflow_behavior: z
      .enum(["allow", "cancel_oldest", "reject_new"])
      .default("allow"),
  })
  .strict();
...
export function stripWorkflowSchema(
  manifest: WorkflowManifest,
): Omit<WorkflowManifest, "$schema"> {
...
export function inferWebhookTrigger(
  manifest: WorkflowManifest,
): WebhookTrigger | undefined {
  return manifest.triggers.find(
    (trigger): trigger is WebhookTrigger => trigger.type === "webhook",
  );
}
