...
import { getAudioProxyPath, getVncProxyPath, SAMPLE_RATE } from "./utils";
...
export class WindowPanel implements vscode.Disposable {
    ...
    private onResizeCallback: ((width: number, height: number) => void) | undefined;
    ...
    constructor(
        ...
        if (existingPanel) {
            ...
            this.panel = vscode.window.createWebviewPanel(WindowPanel.VIEW_TYPE, initialTitle, vscode.ViewColumn.Beside, {
                enableScripts: true,
    ...
    setNonResizable(width: number, height: number) {
        this.panel.webview.postMessage({ type: "set-non-resizable", width, height });
    }

    updateStream(wsPort: number): void {
        if (wsPort === this.wsPort) {
            return;
        }
        ...
        this.panel.webview.postMessage({
            type: "reconnect",
            wsPort,
            vncProxyPath: getVncProxyPath(wsPort),
        });
    }
    ...
    private buildHtml(): string {
        const rfbJs = this.panel.webview.asWebviewUri(vscode.Uri.joinPath(this.extensionUri, "media", "novnc-rfb.js"));
        const windowJs = this.panel.webview.asWebviewUri(vscode.Uri.joinPath(this.extensionUri, "media", "window.js"));
        ...
        const cfg = JSON.stringify({
            ...
            vncProxyPath: getVncProxyPath(this.wsPort),
            audioProxyPath: getAudioProxyPath(),
