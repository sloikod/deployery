...
export class DesktopManager implements vscode.Disposable {
   ...
   */
  private readonly pendingRestorePanels = new Map<
    number,
    vscode.WebviewPanel
  >();
  ...
  private openPanel(win: WindowInfo) {
    ...
    if (existing) {
      ...
      this.log.info(
        `Reconnecting panel con_id=${win.conId} wsPort=${win.wsPort}`,
      );
      existing.updateStream(win.wsPort, true);
      return;
    }
  ...
  private registerPanel(panel: WindowPanel, conId: number) {
    ...
    panel.onDidPanelResize((width, height) => {
      this.session
        ?.resizeWindow(conId, width, height)
        .catch((e) => this.log.warn(`Resize failed con_id=${conId}: ${e}`));
    });
  ...
  private async startAudioPipeline() {
    ...
    try {
      ...
      await runSilent(
        `XDG_RUNTIME_DIR=${PULSE_RUNTIME} pulseaudio --daemonize=yes --exit-idle-time=-1`,
      );
      await sleep(500);
      await run(
        `XDG_RUNTIME_DIR=${PULSE_RUNTIME} pactl load-module module-null-sink sink_name=virtual_sink`,
      );
      await run(
        `XDG_RUNTIME_DIR=${PULSE_RUNTIME} pactl set-default-sink virtual_sink`,
      );
      await run(
        `XDG_RUNTIME_DIR=${PULSE_RUNTIME} pactl load-module module-simple-protocol-tcp port=${PULSE_TCP_PORT} source=virtual_sink.monitor record=true playback=false rate=${SAMPLE_RATE} channels=1 format=s16le`,
      );
    } catch (e) {
      this.log.warn(`Audio pipeline setup failed: ${e}`);
      return;
    }
    this.websockifyProcess = spawn(
      "websockify",
      ["0.0.0.0:" + AUDIO_WS_PORT, `127.0.0.1:${PULSE_TCP_PORT}`],
      {
        stdio: "ignore",
      },
    );
