...
function collectWindowsWithOutput(
  ...
  return [
    ...
    ...(node.floating_nodes ?? []).flatMap((c) =>
      collectWindowsWithOutput(c, out),
    ),
  ];
}
...
function getSwaySocketOrder(name: string): number {
  const match = name.match(/sway-ipc\.\d+\.(\d+)\.sock$/);
  return match ? Number.parseInt(match[1], 10) : -1;
}

function getOrderedSwaySockets(files: string[]): string[] {
  return files
    .filter((file) => file.startsWith("sway-ipc."))
    .sort(
      (left, right) =>
        getSwaySocketOrder(right) - getSwaySocketOrder(left) ||
...
export class SwaySession implements vscode.Disposable {
  ...
  async start(): Promise<void> {
    ...
    const dbusAddress = await new Promise<string>((resolve) => {
      const dbus = spawn(
        "dbus-daemon",
        ["--session", "--fork", "--print-address"],
        {
          stdio: ["ignore", "pipe", "ignore"],
        },
      );
      let addr = "";
      dbus.stdout?.on("data", (d: Buffer) => {
        addr += d.toString();
      });
    ...
    if (dbusAddress) {
      ...
      fs.mkdirSync(
        DEPLOYERY_ENV_FILE.substring(0, DEPLOYERY_ENV_FILE.lastIndexOf("/")),
        { recursive: true },
      );
      fs.appendFileSync(
        DEPLOYERY_ENV_FILE,
    ...
    // Reuse existing Sway if alive. In production Sway is pre-started by
    // entrypoint so we poll until its sockets appear (up to 10s).
    this.log.info("Waiting for Sway session...");
    const existing = await this.findExistingSwaySocket(10_000);
    if (existing) {
    ...
    const swaySocket = getOrderedSwaySockets(files)[0];
    const waylandSocket = files.find((f) => f.startsWith("wayland-"));

    if (!swaySocket || !waylandSocket) {
      throw new Error(
        `Sway sockets not found in ${SWAY_RUNTIME_DIR}: ${files.join(", ")}`,
      );
    }
  ...
  /**
   * Poll for an existing live Sway session. In production Sway is pre-started
   * by entrypoint so we wait up to timeoutMs for its sockets to appear.
   */
  private async findExistingSwaySocket(
    timeoutMs = 0,
  ): Promise<{ socketPath: string; waylandSocket: string } | null> {
    const deadline = Date.now() + timeoutMs;
    let attempt = 0;
    do {
      attempt++;
      try {
        const files = fs.existsSync(SWAY_RUNTIME_DIR)
          ? fs.readdirSync(SWAY_RUNTIME_DIR)
          : [];
        const waylandFile = files.find((f) => f.startsWith("wayland-"));
        this.log.info(
          `Sway check #${attempt}: runtime=${files.join(", ") || "(empty)"}`,
        );
        if (waylandFile) {
          for (const swayFile of getOrderedSwaySockets(files)) {
            ...
            try {
              ...
              this.log.info(
                `Sway check #${attempt}: stale socket ignored ${swayFile}`,
              );
            }
          }
        }
      } catch (e) {
        this.log.info(`Sway check #${attempt} failed: ${e}`);
      }
      if (Date.now() < deadline) await sleep(500);
  ...
  private async reconstructWindowsFromTree(): Promise<void> {
    ...
    for (let attempt = 1; attempt <= 2; attempt++) {
      ...
      this.log.info(
        `get_tree returned 0 windows on attempt ${attempt}, retrying in 500ms`,
      );
      await sleep(500);
    ...
    const outputSummary = (tree?.nodes ?? [])
      ...
      .join(", ");
    this.log.info(
      `Reconstructing ${entries.length} window(s) - outputs: [${outputSummary || "none"}]`,
    );
    for (const { outputName, win } of entries) {
      ...
      this.log.info(
        `Reconstructed: con_id=${win.id} output=${outputName} wsPort=${wsPort}`,
      );
      this.listener.onWindowMapped(this, {
        conId: win.id,
        wsPort,
        title,
        appId,
  ...
  private async detectXWaylandDisplay(): Promise<void> {
    const trigger = spawn(
      "bash",
      ["-c", "DISPLAY=:0 xdpyinfo >/dev/null 2>&1"],
      {
        stdio: "ignore",
        detached: true,
      },
  ...
  private subscribeToEvents(): void {
    ...
    this.eventSub.on("exit", (code) => {
      if (!this.disposed) {
        this.log.warn(
          `Sway event subscription exited (${code}), restarting in 1s`,
        );
  ...
  private handleEvent(json: string): void {
    ...
    try {
      event = JSON.parse(json) as SwayWindowEvent;
    } catch {
    ...
    const { change } = event;
    this.log.trace(
      `Sway window event: change="${change}" con_id=${container.id} type="${container.type}" app_id="${container.app_id ?? container.window_properties?.class ?? ""}" floating="${container.floating}" title="${container.name ?? ""}"`,
    );
  ...
  private async handleNewWindow(container: SwayNode): Promise<void> {
    ...
    if (this.windows.has(conId) || this.pending.has(conId) || this.disposed)
      return;

    const appId: string =
      container.app_id ?? container.window_properties?.class ?? "app";
    const title: string = container.name ?? appId;
    const pid: number = container.pid ?? 0;

    this.pending.add(conId);
    this.log.info(
      `New window: con_id=${conId} pid=${pid} app_id="${appId}" title="${title}"`,
    );
  ...
  private async promoteWindow(
    ...
    const before: string[] = (JSON.parse(beforeJson) as SwayOutput[]).map(
      (o) => o.name,
    );
    ...
    const after: string[] = (JSON.parse(afterJson) as SwayOutput[]).map(
      (o) => o.name,
    );
    ...
    await run(
      `swaymsg '[con_id=${conId}] move container to output ${outputName}'`,
    );
    ...
    this.log.info(
      `Window pending: con_id=${conId} output=${outputName} wsPort=${wsPort} - awaiting first panel-resize`,
    );
  ...
  private async startStream(
    ...
    const wayvnc = spawn(
      ...
      [
        ...
        "60",
        "-S",
        `/tmp/wayvnc-ctl-${vncPort}`, // unique ctl socket per instance - avoids "already running" error
        "0.0.0.0",
        String(vncPort),
      ],
      { stdio: ["ignore", "pipe", "pipe"] },
    ...
    wayvnc.on("exit", (code) => {
      this.log.info(
        `wayvnc exited: con_id=${conId} output=${outputName} code=${code}`,
      );
    ...
    if (wayvnc.exitCode !== null) {
      this.log.warn(
        `wayvnc already exited during stream start - skipping panel`,
      );
      websockify.kill("SIGTERM");
      return;
    }

    this.windows.set(conId, {
      outputName,
      wayvnc,
      websockify,
      vncPort,
    ...
    this.log.info(
      `Stream started: con_id=${conId} output=${outputName} vncPort=${vncPort} wsPort=${wsPort}`,
    );
    ...
    if (pendingColor) {
      ...
      this.log.info(
        `bg-color applied (deferred): output=${outputName} color="${pendingColor}"`,
      );
      runSilent(
        `swaymsg output ${outputName} bg "${pendingColor}" solid_color`,
      );
    }
    ...
    if (width > 0 && height > 0) {
      (async () => {
        await sleep(100);
        await runSilent(
          `swaymsg output ${outputName} mode ${width}x${height - 1}`,
        );
        await sleep(50);
  ...
  private handleWindowClose(container: SwayNode): void {
    ...
    if (pending) {
      this.log.info(
        `Pending window closed before stream started: con_id=${conId}`,
      );
  ...
  private handleWindowGeometryChange(container: SwayNode): void {
    ...
    this.log.info(
      `Window geometry changed: con_id=${conId} ${state.width}x${state.height} -> ${w}x${h}`,
    );
    state.width = w;
    state.height = h;
  ...
  async resizeWindow(
    ...
    if (pending) {
      ...
      this.pendingWindows.delete(conId);
      this.log.info(
        `First resize: con_id=${conId} output=${pending.outputName} → ${width}x${height}`,
      );
      ...
      if (winState && winState.width > 0 && winState.height > 0) {
        ...
        this.log.info(
          `Window state: con_id=${conId} → ${winState.width}x${winState.height} (${match ? "matches output" : "correcting output"})`,
        );
        if (!match) {
          streamW = winState.width;
          streamH = winState.height;
          await run(
            `swaymsg output ${pending.outputName} mode ${streamW}x${streamH}`,
          );
          this.nonResizable.add(conId);
          if (!this.disposed)
            this.listener.onWindowNonResizable(this, conId, streamW, streamH);
        }
      } else {
        this.log.warn(
          `Window state: con_id=${conId} → null, starting at panel size`,
        );
      }
    ...
    this.log.info(
      `Resize: con_id=${conId} output=${state.outputName} → ${width}x${height}`,
    );
    state.width = width;
    state.height = height;
  ...
  setOutputBackground(conId: number, color: string): void {
    ...
    if (!state) {
      this.log.info(
        `bg-color queued for con_id=${conId}: "${color}" (stream not yet started)`,
      );
      this.pendingBgColors.set(conId, color);
      return;
    }
    this.log.info(
      `bg-color applied: output=${state.outputName} color="${color}"`,
    );
