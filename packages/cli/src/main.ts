...
const BASE_URL = (process.env.DEPLOYERY_BASE_URL ?? "http://localhost:3131").replace(/\/$/, "");
const API_KEY = process.env.DEPLOYERY_API_KEY;
const WORKFLOWS_DIR = path.join(process.env.HOME ?? process.cwd(), "Desktop", "Workflows");
...
type StoredWorkflow = {
    ...
    name: string;
    manifest: {
        steps: Array<{ name: string; pinned_input?: unknown; pinned_output?: unknown }>;
        triggers: unknown[];
        settings?: unknown;
    };
};
...
function resolveWorkflowFiles(filter?: string): Array<{ name: string; path: string; manifest: ReturnType<typeof parseWorkflowManifest> }> {
    ...
    if (fs.existsSync(filter) && fs.statSync(filter).isFile()) {
        return workflows.filter((workflow) => workflow.path === path.resolve(filter));
    }

    const byName = workflows.filter((workflow) => workflow.name === filter || path.basename(workflow.path) === filter);
    if (byName.length === 0) {
        die(`Error: No workflow named "${filter}" found in ${process.cwd()} or ${WORKFLOWS_DIR}`);
    }
...
async function getWorkflowByName(name: string): Promise<StoredWorkflow | undefined> {
...
async function push(filter?: string) {
    ...
    for (const workflow of workflows) {
        if (!VALID_NAME.test(workflow.name)) {
            die(`Error: "${path.basename(workflow.path)}" must use lowercase kebab-case`);
        }
...
async function pull(filter?: string) {
    ...
    for (const workflow of selected) {
        ...
        const filePath = targetPath ?? path.join(WORKFLOWS_DIR, `${workflow.name}.json`);
        fs.writeFileSync(filePath, `${JSON.stringify(manifest, null, 2)}\n`);
        console.log(`pulled: ${workflow.name} -> ${path.relative(process.cwd(), filePath)}`);
    }
}
...
async function trigger(name: string, data: string | undefined) {
    ...
    const response = await apiFetch<StoredRun>(`/api/v1/workflows/${workflow.id}/trigger`, {
        method: "POST",
        body: JSON.stringify({
            data: data ?? null,
        }),
...
async function run(name: string, data: string | undefined) {
    ...
    const run = await apiFetch<StoredRun>(`/api/v1/workflows/${workflow.id}/trigger`, {
        method: "POST",
        body: JSON.stringify({
            data: data ?? null,
        }),
    ...
    while (true) {
        ...
        const currentRun = await apiFetch<StoredRun>(`/api/v1/workflows/runs/${run.id}`);
        ...
        const logs = await apiFetch<Array<{ stream: string; chunk: string; createdAt: number }>>(
        ...
        if (currentRun.status === "completed") {
            const duration =
                currentRun.finishedAt !== null ? formatDuration(currentRun.finishedAt - currentRun.startedAt) : "-";
            console.log("");
        ...
        if (currentRun.status === "failed" || currentRun.status === "cancelled") {
            const duration =
                currentRun.finishedAt !== null ? formatDuration(currentRun.finishedAt - currentRun.startedAt) : "-";
            die(`Run ${currentRun.status} in ${duration}: ${currentRun.error ?? "unknown error"}`);
        }
    }
}
...
async function listRuns(name?: string) {
    const workflows = await listWorkflows();
    const selected = name ? workflows.filter((workflow) => workflow.name === name) : workflows;
    if (selected.length === 0) {
        die(`Error: Workflow "${name}" not found`);
    }
    ...
    for (const workflow of selected) {
        const runs = await apiFetch<StoredRun[]>(`/api/v1/workflows/${workflow.id}/runs?limit=10`);
        runs.forEach((run) => {
            rows.push([
                workflow.name,
                run.id.slice(-8),
    ...
    const headers = ["WORKFLOW", "RUN ID", "STATUS", "STARTED", "DURATION", "TRIGGER"];
...
async function webhook(name: string) {
    ...
    const urls = await apiFetch<{ trigger: string }>(`/api/v1/workflows/${workflow.id}/webhook-urls`);
    console.log(urls.trigger);
}

function updatePinnedValue(name: string, stepName: string, side: "input" | "output", value: unknown) {
    const local = resolveWorkflowFiles(name);
    if (local.length === 0) {
        die(`Error: Workflow "${name}" not found locally`);
    }
...
async function pin(name: string, stepName: string, side: "input" | "output") {
    ...
    const runs = await apiFetch<StoredRun[]>(`/api/v1/workflows/${workflow.id}/runs?limit=1&status=completed`);
    if (runs.length === 0) {
    ...
    updatePinnedValue(name, stepName, side, side === "output" ? stepRun.output : stepRun.input);
...
async function main() {
    ...
    switch (command) {
        ...
        case "webhook":
            ...
            return;
        case "pin":
            if (!args[0] || !args[1] || (args[2] !== "input" && args[2] !== "output")) {
