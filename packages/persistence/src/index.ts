...
export function loadPersistenceOptionsFromEnv(
  defaultSqlitePath = "./deployery.sqlite",
  ...
  if (type === "sqlite") {
    return {
      type,
      sqlitePath:
        readEnv("DB_SQLITE_PATH") ??
        readEnv("DEPLOYERY_SQLITE_PATH") ??
        defaultSqlitePath,
    };
  }
...
function normalizeOptions(
  optionsOrPath: PersistenceStoreOptions | string,
): PersistenceStoreOptions {
...
export class PersistenceStore {
  ...
  async upsertWorkflow(
    ...
    if (existing) {
      await this.db.run(
        `UPDATE workflows
                 ...
                 WHERE id = ?`,
        [JSON.stringify(manifest), 1, timestamp, existing.id],
      );
    ...
    await this.db.run(
      `INSERT INTO workflows (id, name, enabled, manifest, created_at, updated_at)
             VALUES (?, ?, ?, ?, ?, ?)`,
      [workflowId, name, 1, JSON.stringify(manifest), timestamp, timestamp],
    );
    return (await this.getWorkflowById(workflowId)) as WorkflowRecord;
  }

  async replaceScheduledTriggers(
    workflowId: string,
    triggers: CreateScheduledTriggerInput[],
  ): Promise<ScheduledTriggerRecord[]> {
    await this.db.run(`DELETE FROM scheduled_triggers WHERE workflow_id = ?`, [
      workflowId,
    ]);
  ...
  async listDueScheduledTriggers(
    timestamp: number,
  ): Promise<ScheduledTriggerRecord[]> {
    const rows = await this.db.all<ScheduledTriggerRow>(
      `SELECT
  ...
  async updateScheduledTriggerAfterFire(
    id: string,
    nextRunAt: number | null,
    firedAt: number,
  ): Promise<void> {
  ...
  async listActiveRunsByWorkflow(
    workflowId: string,
  ): Promise<WorkflowRunRecord[]> {
