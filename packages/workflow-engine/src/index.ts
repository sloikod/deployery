...
function computeNextRunAt(trigger: WorkflowTrigger, from: Date): number | null {
  ...
  if (trigger.at) {
    ...
    return Number.isFinite(atTimestamp) && atTimestamp > from.getTime()
      ? atTimestamp
      : null;
  }
...
export class WorkflowEngine {
  ...
  constructor(options: WorkflowEngineOptions) {
    this.store = createPersistenceStore(
      options.persistence ?? options.sqlitePath ?? "./deployery.sqlite",
    );
  ...
  async resumeRun(
    ...
    await this.store.appendRunLog(
      ...
      updated.workflowId,
      "system",
      `Resumed run from token ${resumeToken}.`,
    );
  ...
  async getWebhookUrls(
  ...
  ): Promise<{ trigger: string; resumeTemplate: string }> {
    const workflow = await this.store.getWorkflowById(workflowId);
    if (!workflow) {
      throw new Error("workflow not found");
    }
    ...
    const secretSegment = webhookTrigger?.secret
      ? `/${encodeURIComponent(webhookTrigger.secret)}`
      : "";
  ...
  private computeScheduledTriggers(
  ...
  private async tickScheduler(): Promise<void> {
    ...
    for (const scheduledTrigger of await this.store.listDueScheduledTriggers(
      nowValue,
    )) {
      const workflow = await this.store.getWorkflowById(
        scheduledTrigger.workflowId,
      );
      if (!workflow) {
        continue;
      }
      ...
      await this.store.updateScheduledTriggerAfterFire(
        scheduledTrigger.id,
        nextRunAt,
        nowValue,
      );
      ...
      try {
        ...
        this.logger.warn(
          `Scheduled trigger ${scheduledTrigger.id} failed: ${message}`,
        );
      }
    }
    ...
    for (const run of await this.store.listRecoverableRuns()) {
      if (
        run.status === "waiting" &&
        run.waitingFor === "schedule" &&
        run.resumeAt !== null &&
        run.resumeAt <= nowValue
      ) {
        const stepRuns = [...run.stepRuns];
  ...
  private async executeRun(runId: string): Promise<void> {
    ...
    await this.store.appendRunLog(
      ...
      run.workflowId,
      "system",
      `Run started for workflow "${workflow.name}".`,
    );

    let previousOutput = run.input;
    for (
      let stepIndex = run.currentStep;
      stepIndex < workflow.manifest.steps.length;
      stepIndex += 1
    ) {
      ...
      if ("pinned_output" in step && step.pinned_output !== undefined) {
        ...
        await this.store.appendRunLog(
          ...
          run.workflowId,
          "system",
          `Step "${step.name}" used pinned output.`,
        );
      ...
      try {
        ...
        const failedStepRuns = [
          ...((await this.store.getRun(run.id))?.stepRuns ?? stepRuns),
        ];
    ...
    await this.store.appendRunLog(
      ...
      run.workflowId,
      "system",
      `Run completed for workflow "${workflow.name}".`,
    );
  }
  ...
  private async executeCommandStep(
    ...
    return new Promise<JsonValue>((resolve, reject) => {
      ...
      const appendLog = (stream: WorkflowLogStream, chunk: string) => {
        ...
        void this.store
          .appendRunLog(run.id, run.workflowId, stream, chunk)
          .catch((error) => {
            const message =
              error instanceof Error ? error.message : String(error);
            this.logger.warn(
              `Failed to persist ${stream} log chunk for run ${run.id}: ${message}`,
            );
          });
      };
  ...
  private async executeWorkflowStep(
    ...
    const timeoutAt = step.timeout_seconds
      ? Date.now() + step.timeout_seconds * 1_000
      : null;
    while (true) {
      await sleep(500);
      ...
      if (run.status === "failed" || run.status === "cancelled") {
        throw new Error(
          run.error ?? `Nested workflow ${step.workflow} did not complete`,
        );
      }
  ...
  private async executeWaitStep(
    ...
    const stepRuns = [
      ...((await this.store.getRun(run.id))?.stepRuns ?? run.stepRuns),
    ];
    stepRuns[stepIndex] = {
      ...stepRuns[stepIndex],
      status: "waiting",
    ...
    await this.store.appendRunLog(
      ...
      run.workflowId,
      "system",
      `Run is waiting for ${step.mode} resume.`,
    );
  ...
  private async executeScheduleStep(
    ...
    const resumeAt =
      step.at !== undefined
        ? new Date(step.at).getTime()
        : Date.now() + (step.delay_seconds ?? 0) * 1_000;
    ...
    await this.store.appendRunLog(
      ...
      run.workflowId,
      "system",
      `Run scheduled to resume at ${new Date(resumeAt).toISOString()}.`,
    );
  ...
  private async executeOutgoingWebhookStep(
    ...
    const timeoutMs = step.timeout_seconds
    ...
    try {
      ...
      await this.store.appendRunLog(
        ...
        run.workflowId,
        "system",
        `Webhook step "${step.name}" returned ${response.status}.`,
      );
      return asJsonValue({
  ...
  private verifyWebhookTrigger(
    trigger: WebhookTrigger,
    context: WebhookRequestContext,
  ): void {
    if (
      trigger.method &&
      trigger.method.toUpperCase() !== context.method.toUpperCase()
    ) {
    ...
    if (trigger.verification.mode === "hmac") {
      const signatureHeader =
        trigger.verification.signature_header?.toLowerCase();
      ...
      const expectedSignature = createHmac(algorithm, secret)
        ...
        .digest("hex");
      const prefixedExpected = trigger.verification.prefix
        ? `${trigger.verification.prefix}${expectedSignature}`
        : expectedSignature;
