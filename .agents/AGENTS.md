...
3. **No slop** - test every behavior change, keep coverage high (`95%+` target, no fake padding), and run `pnpm check` (`pnpm lint && pnpm check-types && pnpm test:run`) before done
